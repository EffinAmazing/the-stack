    ...
    async.waterfall(
      [
        ...
        (blueprint, cb) => {
          ...
          this._toolsNodes
            .getNodesByBlueprint(data.blueprint.id)
            .then((result) => {
              data.nodes = result;
              cb(null, data);
            })
        ...
        (data, cb) => {
          if (data.nodes.length !== 0) {
            ...
            ToolsServices.getToolsOfDomain(domain)
              .then((result) => {
                const list = result.tech;
                data.blueprint["spend"] = result.spend;
                console.log(data.blueprint);
                this._bluePrints
                  .updateOne(data.blueprint.id, { spend: result.spend })
                  .then((result) => {})
  ...
  httpRequest(params, postData) {
    return new Promise(function (resolve, reject) {
      var req = http.request(params, function (res) {
        ...
        //this array is used to bypass domains that have already been imported, and need to be obfuscated
        ...
        [
          ...
          (blueprint, cb) => {
            if (blueprint.userId.toString() == user._id.toString()) {
              ...
              this._bluePrintsAccess
                .hasUserAccess(user._id, blueprint.id)
                .then((result) => {
                  if (result) {
    ...
    if (emails && pathFront && user) {
      async.waterfall(
        [
          (cb) => {
            this._users
              .verifyListBluePrintConnection(emails, blueprintId, user._id)
              .then((result) => {
                cb(null, result);
              })
              .catch((err) => {
          ...
          (filtered, cb) => {
            this._bluePrints
              .sendInviteUserToBluePrint(filtered, pathFront, blueprintId)
    ...
    if (blueprintId && user) {
      ...
      async.waterfall(
        [
          (cb) => {
            this._bluePrints
              .copyBluePrint(blueprintId, user._id)
              .then((blueprint) => {
                cb(null, blueprint);
              })
              .catch((err) => {
          ...
          (blueprint, cb) => {
            this._toolsNodes
              .copyNodes(blueprintId, blueprint.id)
              .then((keysTable) => {
                cb(null, blueprint, keysTable);
              })
              .catch((err) => {
                cb(err);
              });
          ...
          (blueprint, keysTable, cb) => {
            this._arrows
              .copyArrows(blueprintId, blueprint.id, keysTable)
              .then(() => {
                cb(null, blueprint);
              })
              .catch((err) => {
